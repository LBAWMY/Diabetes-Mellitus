...
lowValue = np.percentile(train['血糖'],20)
print('<'+str(19),train.shape)
print('<'+str(lowValue),train[train['血糖']<=lowValue].shape)
if OFFLINE_BUTTON == 1:
...
train['血糖'] = train['血糖'].apply(lambda x: 1 if x<=lowValue else 0)
train_y = train['血糖']
...
test_result["lp30_prob"] = test_y
test_result.to_csv("lp30_prob.csv",index=None,encoding='utf-8')

print('lower value:  '+str(lowValue))
