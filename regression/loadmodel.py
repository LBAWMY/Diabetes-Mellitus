...
xgb_reg = xgb.Booster({'nthread':8})
xgb_reg.load_model('../model/xgb_reg.model')

#predict test set
test_y = xgb_reg.predict(dtest)
test_result = pd.DataFrame(test_id,columns=["id"])
test_result["score_all"] = test_y
test_result.to_csv("xgb_preds1.csv",index=None,encoding='utf-8')